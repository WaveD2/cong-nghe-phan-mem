...
const ForgotPassword = () => {
  ...
  const { requestForgotPassword,confirmForgotPassword,forgotPassword} = useAuth();
  ...
  const handleRequestOtp = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setCountdown(response.data.ttl || 0);
        setStep("verify");
      }
    } catch (err) {
      setError(
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const response = await confirmForgotPassword(email, otp);
